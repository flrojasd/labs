...
function averageWordLength(words) {
  ...
  let averageWord = totalLenght / words.length;
  return averageWord 

 }
 let averageWord = averageWordLength(wordsArr);
...
let word ="machine";
let wordExist = doesWordExist (wordsFind,word);
...
function howManyTimes(arr,wordToCount) {
  ...
  for (let i=0; i<arr.length; i++) {
    if (arr[i]=== wordToCount) { count++;}
  }
  return count;
}
let wordToCount = 'matter'
let numberOfWords = howManyTimes (wordsCount,wordToCount) ; 
